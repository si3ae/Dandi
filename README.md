...
> spot in financial and tax systems across cash-heavy economies.
...
real vendor pilot is still seeking an institutional partner. Server-side usage
logging (Supabase) was not enabled during the event, so per-request server
metrics for that run are unavailable; the cost figures above come from the API
dashboards.
